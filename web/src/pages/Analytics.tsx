...
const Analytics = () => {
  ...
  const filterByMonthYear = (transactionsList, month, year) =>
    transactionsList.filter((t) => {
      const d = new Date(t.date);
      return d.getMonth() === month && d.getFullYear() === year;
    });
  // Get filtered transactions for the selected month/year
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-background/90 border border-border p-2 rounded-md shadow-md">
          <p className="font-medium">{`${data.name}: ${formatTooltipValue(
            data.value
          )}`}</p>
        </div>
      );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Daily Spending for {format(currentDate, "MMMM yyyy")}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-[320px]">
              {" "}
              {/* Matching height with other chart */}
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={personalDailySpending}
                  margin={{
                    top: 10,
                    right: 30,
                    left: 20,
